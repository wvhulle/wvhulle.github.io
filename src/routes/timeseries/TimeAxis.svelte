<script lang="ts">
	...
	$: timeDomain = extent(data, ({ x }): number => x.getTime()) as [number, number];
	...
	export let start = timeVariable ? dayjs(data[0].x as string | Date).toDate() : undefined;
...
<g transform="translate(0, {height - margin.bottom})">
	{#if timeScale && end !== undefined && start !== undefined}
		<text transform="translate({timeScale((end.getTime() + start.getTime()) / 2)},+50)">
			{timeVariable}
		</text>
	{:else}
	...
	{#if timeTicks && timeScale}
		{#each timeTicks as x, i (x)}
			...
			>
				...
				<text fill="currentColor" y="9" dy="1em" text-anchor="middle" alignment-baseline="central">
					<tspan>
						{xLabel(x)}
					</tspan>
				</text>
